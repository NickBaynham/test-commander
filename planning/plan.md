...
- [x] 7.1 — Skill scaffolds (`tc-run` + `tc-quality-report` + `tc-evidence`) and seeded-results fixture
- [x] 7.2 — `/tc:run` (execution + result capture; execution refused under pytest)
- [x] 7.3 — `tc-evidence` indexer + evidence policy + run auto-index wiring
- [x] 7.4 — `/tc:analyze-results` (failure triage + flaky detection)
- [x] 7.5 — `/tc:report` (quality report + history snapshot) + `test-map` downstream resolution
- [x] 7.6 — `/tc:quality-gate` (PASS / WARN / FAIL)
- [x] 7.7 — Documentation pass (`running-tests.md`, `quality-report.md`, customization-guide Phase 7 schema)
- [x] 7.8 — Testing finalization (cap bump 6 → 7 + integration smoke + `PHASE_OWNERSHIP` narrowing)
