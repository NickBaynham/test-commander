...
- [x] 8.2 — `/tc:learn` + the `tc-lesson/v1` schema (shared inbox-append + dedup engine)
- [x] 8.3 — `/tc:learn-from-failures` (reads Phase-7 `runs/*/analysis.md`)
- [x] 8.4 — `/tc:learn-from-exploration` (reads Phase-4 exploration-notes/sessions)
- [x] 8.5 — `/tc:learn-from-feedback` (reads resolved open-questions / feedback)
- [x] 8.6 — `/tc:review-lessons` (classify inbox → accepted/rejected/needs-human-review)
- [x] 8.7 — `/tc:promote-lessons` (governed promotion; `--apply` human gate; never rewrites shipped methodology)
- [x] 8.8 — Documentation pass (`learning-loop.md`, references, customization Phase 8 schema)
- [x] 8.9 — Testing finalization (cap bump 7 → 8 + integration smoke)
