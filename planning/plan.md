...
- [x] **5.2** — `/tc:generate-bdd` (TDD, generation only) + umbrella `bdd-generation.md` methodology + `feature-template.feature` + summary template; emits Gherkin with `@area:`/`@req:`/`@cs:` linkage tags; declares the `Scenario` dataclass mirroring the enriched-test-idea shape; rebuilds `bdd/index.md` (review wiring deferred to 5.3)
- [x] **5.3** — `/tc:review-bdd` (TDD) + shared `review_features()` + wire generate-bdd's auto-run (`--no-review`) + `bdd-quality-review.md`; six-category universal rubric; `[bdd-review]` gap signals with the Phase-2 dedup contract
- [x] **5.4** — `/tc:traceability-map` (TDD) — extract shared `traceability_render.py` from `requirements_coverage.py` first (Phase-2 tests stay green); rebuild `requirements-map.md` (BDD-scenario column) + `test-map.md` with `pending` downstream links
- [x] **5.5** — Documentation pass: author `docs/user-guide/generating-bdd.md`; update command-reference, workspace-reference (bdd/ + traceability/ reconciliation + linkage-tag convention), customizing-for-your-project (Phase 5 tag-namespace schema + three project-shape worked examples), status lines
- [x] **5.6** — Testing finalization: bump `DEFAULT_PHASE_CAP` to 5; author `test_phase_5_integration.py` (in-process, Phase 2 → 3 → 4 → 5 order); assert `product-knowledge/` + `test-ideas/` untouched and shared-renderer regression; `/tc:next` advances past `/tc:generate-bdd`
