...
- **Step 10.5.13 — sign-off.** Six sub-sub-steps. Cold-user walkthrough: `make uninstall` → `make install` clean (17 skills `PRESENT`, `UNEXPECTED=0`), then drove the console end to end against the mock adapter — `/api/execute` denied an unsafe Viewer request (`blocked`, never reached the agent), approved an Automation-Engineer code-write (`executed`, one audit entry), and `/api/chat "run the tests now"` executed nothing and wrote no audit entry. **The walkthrough found no bugs this time** — unlike Phase 10's `:ro`-mount surprise — because the whole pipeline is in-process Python fully covered by the hermetic suite; there's no container/mount layer the suite can't see (the one shell-out, the Claude adapter, is refused under pytest and isn't exercised by the MVP console, which uses the mock). Test-first gate `tests/test_phase_10_5_signoff.py` (15 tests, floor `>= 950`, cap regex `([\d.]+)` for the decimal) landed RED on the four closing assertions, GREEN after. Annotated `phase-10.5` tag confirmed absent, then pushed. **Phase retrospective: the xfail-strict security spine was the single best decision** — four RED acceptance tests written at 10.5.1 turned the whole 13-step build into 'make the next security property go green', and `make verify` stayed green at every one of the 13 commits.
- **Step 10.5.12 — testing finalization.** Bumped `DEFAULT_PHASE_CAP` 10 → **10.5** (a float); `tc-governance` flips `UNEXPECTED → PRESENT`, so all seventeen skills report `PRESENT` with `UNEXPECTED=0`. **The decimal cap doesn't break the prior sign-off tests:** they read `DEFAULT_PHASE_CAP:\s*float\s*=\s*(\d+)` and assert `int >= N`; against `10.5` the `\d+` matches `10`, so phase-8/9/10 still see `10 >= their N`. The 10.5.13 sign-off test uses `([\d.]+)` + `float()` to assert `>= 10.5`. Added `tests/test_phase_10_5_integration.py` — the consolidated end-to-end smoke that drives all four security properties through the real pipeline in one workspace (block → deny → approve+execute+validate+audit → bypass-refused), plus secret redaction, the Claude adapter gated identically, and `PHASE_OWNERSHIP["10.5"] == ["policy", "audit"]`. The focused per-property spine (`test_phase_10_5_security.py`) stays as the RED-first acceptance tests; the integration file is the cohesive proof they hold together end to end.
- **Step 10.5.11 — documentation pass.** No code changes; `make verify` clean (985 tests). **Five of the six governance docs already existed as design specs** (written 2026-05-26/27, ahead of implementation) — so the doc pass was mostly *reconciliation*, not authoring: I aligned `agent-adapters.md` (the interface block showed `ExecutionHandle`/`AgentEvent` sketches; updated to the shipped `execute_command(instruction)->ExecutionResult` signatures + `require_planned`/no-backdoor) and `security-and-permissions.md` (the audit schema showed `id`/`permission_level`/`artifacts_created`/`tests_run:[...]`; updated to the shipped `level`/`artifacts`/`tests_run:bool` field names). **Lesson: design docs written ahead of code drift from the implementation — the doc pass must diff them against what shipped, not just add a new file.** Authored the one missing doc, `docs/user-guide/governance.md` (the tester-facing guide with verbatim approval-card / deny / approve / audit output). The customization-guide Phase 10.5 entry is the phase's genuinely-extensible surface (roles in `permissions.yaml`, approval policy in `approvals.yaml`) with two worked examples — the only phase since Phase 7 to ship a real config surface rather than record 'no new surface'. Flipped the `tc-governance` SKILL Status from the step-by-step deferral framing to 'all components shipped' (the sign-off test asserts no deferral wording).
...
- **Step 11.7 (sign-off).** The six-sub-sub-step sign-off held: the cold-user walkthrough (`make uninstall` → `make install` clean, `verify_skills: OK PRESENT=18 UNEXPECTED=0`, then both front-ends driven against a fresh consuming project — Runtime API info/plan/block/hold/run and the MCP handshake + gating) found no bugs, because every property had already been verified RED-first across 11.2–11.6. `tests/test_phase_11_signoff.py` (16 tests) landed RED on the five closing markers (lessons-11.7 entry, CHANGELOG complete, plan Completed entry, To Do collapse, README status flip) and GREEN after. The no-deferral assertion was strengthened over 10.5's: it greps the *whole* `tc-mcp/` tree (`rglob("*.md")`), not just SKILL.md, because the one stale deferral line in 11.5 was in a methodology page. Phase retrospective: the spine of Phase 11 was the no-bypass discipline, and two real bypasses fell out of writing adversarial tests rather than happy-path ones — the unattributed approval (11.4) and the privileged-classified-but-unrouted no-op execution (11.6). Both were one-line fixes in the shared pipeline, so both front-ends inherited them; the lesson that recurs is that a shared governance core means a test on one front-end protects all of them, and that "enforce X" sub-steps must send the input the existing tests never thought to send.
- **Step 11.6 (testing finalization).** The consolidated integration test paid off immediately: asserting "no front-end can bypass the pipeline" surfaced a real governance-semantics bug the per-step tests missed — a dangerous *unrouted* request (`delete all evidence`, which the router maps to the read-only plan because there is no destructive `/tc:*` command) "executed" a no-op for any role permitted that level, reporting `executed: true` and writing an audit entry with `requires_approval: false`. Root cause: the read-only early-return keyed off the classified `level`, not `the_plan.command`. The fix (return early when the plan is read-only — there is nothing to execute) is one line and tightens the headline no-bypass guarantee. Lesson: a request's *classified level* (the policy/block signal) and its *executable plan* are two different things — gates that branch on one must not silently use the other. My own Step-11.4 test had encoded the buggy behavior ("destructive runs for a Maintainer"); the integration pass is the right place to catch that, and the correction split it into the honest pair (a real privileged command runs; a destructive request has no executable workflow). The decimal cap (`11`) does not break the prior sign-off tests — their `\d+` regex matches and `11 >= 8/9/10/10.5`. No new extensible surface.
- **Step 11.5 (documentation pass).** `docs/runtime-api.md` existed from Phase 10 with a "there is no mutating route" contract that Phases 10.5 and 11 had since falsified — a reminder that a prior phase's docs can quietly go stale, so a docs pass must re-read the existing file's claims, not just append. Phase 11 ships no new extensible surface (the front-ends inherit `permissions.yaml`/`approvals.yaml`), so per Convention #6 the customization-guide entry records that explicitly rather than inventing a config key. The deferral sweep caught one stale "lands in Step 11.4" line in a methodology page — the sign-off test (11.7) must grep the whole `tc-mcp/` tree, not just SKILL.md. No code; link checker rose 307 → 309 with the two new docs.
- **Step 11.4 (permission gates).** Writing the per-level matrix surfaced that the gates were *already* enforced through the pipeline (6 of 8 tests passed RED) — but probing the approval gate at the REPL before writing assertions (per the prove-the-problem-first discipline) caught a real bypass the happy-path 10.5 tests never exercised: `approve=True` with no `approver` executed a privileged action and recorded an unattributed approval with no approval record. The fix is one boolean (`not (approve and approver)`) in the shared pipeline, so both front-ends inherited it for free, and it was behavior-preserving for every legitimate call (a real approval always has an approver) — the 10.5 suite stayed green. Lesson: when a sub-step's brief is "enforce X," verify X is *actually* enforced against an adversarial input (here, a malformed approval), not just that the happy path works — the gap is always in the input the existing tests didn't think to send. No new extensible surface (the gates enforce the existing seven levels and the existing `approvals.yaml`).
- **Step 11.3 (MCP server + tools).** The lightweight schema-first registry was the right call over the heavy `mcp` SDK: `dispatch()` (handle one `initialize`/`tools/list`/`tools/call` message) is the transport-agnostic, fully-unit-testable core, and `__main__.py` is a trivial stdio wrapper, so the suite stays hermetic and in-process — a `SampleClient` drives `dispatch` exactly as a real MCP client would. Keeping all three tools' execution paths inside `pipeline.handle_request`/`preview` (rather than re-deriving anything) means the MCP front-end inherits every governance guarantee for free; `tc_run_command` is gated identically to the console's `/api/execute`. One coupling to remember: the seeded fixture (created at 11.1 with placeholder tool names `tc_query`/etc.) had to be re-aligned to the real tool names and classification-accurate levels in this sub-step — a fixture written ahead of the implementation will drift, so reconcile it in the step that ships the behavior. No new extensible surface (the tools enforce the existing seven levels).
- **Step 11.2 (Runtime API expansion).** Extracting `pipeline.preview()` (route + plan + classify + allowance) and having both `handle_request` and the new `/api/runtime/plan` route consume it is the right DRY move — the dry-run preview and the enforced decision share one code path and cannot drift. The 10.5 governance tests stayed green through the refactor, confirming the extraction was behavior-preserving. The sharp edge: the intent router maps the keywords `report` and `requirement` to *write* commands (`/tc:report`, `/tc:review-requirements`), so a read-phrased request ("show me the quality report") classifies as `safe-write` on the execute path. This is correct — the console's read path is the GET routes, not `/execute` — but it means the contract tests for the read-only level must use phrasing with no route/classifier keyword (e.g. "what is the current status"), not anything containing `report`/`requirement`. Verified routing/classification at the REPL before fixing the test expectations rather than guessing. No new extensible surface (the Runtime API enforces the existing seven levels; it adds none).
...
- **Step 12.7 (sign-off).** The cold-user walkthrough drove the *real* docker-compose provider (not the mock) through the full lifecycle from the CLI — init → launch → status → sync → export → stop → status — and it worked end to end without Docker because the helpers default to `dry_run=True` and only the non-dry path shells out. That is the payoff of the dry-run-by-default design: a no-Docker machine can exercise the whole command surface safely. `tests/test_phase_12_signoff.py` (17 tests) landed RED on the five closing markers and GREEN after; the no-deferral assertion greps the whole `tc-sandbox/` tree (`rglob`), the lesson carried from Phase 11. Phase retrospective: the seam that made the phase clean was the `SandboxProvider` interface plus `MockSandboxProvider` — every command and the integration test drive the mock, so nothing in the suite touches Docker or a cloud, and the one real shell-out (the docker-compose provider's non-dry path) is refused under pytest, exactly as the Phase-10.5 Claude adapter is. Governance travelling with the sandbox cost one thin module (`run_in_sandbox` → `handle_request`) precisely because Phase 10.5 built the pipeline as the single execution path. One prior-phase test broke at the status flip: `test_phase_11_signoff.py::test_status_line_marks_phase_11_complete` pinned the rolling README status line to the literal "Phase 11 complete", which advancing to "Phase 12 complete" removed — the exact monotonic-assertion anti-pattern the `>=` cap lesson warns about, now applied to the status line. Fixed both the Phase-11 and Phase-12 status-line assertions to extract every "Phase N complete (date)" and assert `max(N) >= 11`/`>= 12` — robust to all future phases. Lesson: a sign-off test that checks the *rolling* status line must assert "phase N or later closed", never the literal phase string, just as cap assertions use `>=`.
- **Step 12.6 (testing finalization).** The integration test consolidated the lifecycle into one workspace run and confirmed the provider-call ordering (`[launch, sync, teardown]`) — the cleanest way to assert "build → publish → teardown sequencing" without a real CI run. Unlike Phases 11's two finalization-surfaced bugs, Phase 12's consolidated test found nothing new: the provider/command/safety/governance layers were already adversarially tested per sub-step, and the integration test is a true smoke (every assertion held first try). The cap bump to 12 is clean for the same reason the 11 bump was — prior sign-off cap regexes are `\d+`/`[\d.]+` and `12 >= 10.5/11`. Lesson reaffirmed: when each sub-step ships with RED-first adversarial tests, the integration pass is confirmation, not discovery — which is the goal.
- **Step 12.5 (documentation pass).** The honest-MVP-limitations section is the load-bearing part of this phase's docs (it is the phase's explicit DoD): the sandbox CI workflow's build/publish/teardown steps are demonstrative placeholders, and saying so plainly — rather than implying a full cloud sandbox ships — is the difference between honest and oversold documentation. Because the customization-guide entry and `docs/user-guide/sandbox.md` already had to land in 12.4 (Convention #6 + link integrity), 12.5 was lighter than a typical docs pass: three architecture docs, a command-reference section, and the status sweep. Cross-doc anchors (`#mvp-limitations-honest`) must match the rendered heading slug — GitHub lowercases, strips punctuation, and hyphenates, so "MVP limitations (honest)" → `mvp-limitations-honest`; the link checker validates file targets but not anchors, so anchor correctness is on the author. No code.
- **Step 12.4 (workflows + safety guards).** Two guard layers, kept separate and both one small module: `safety.py` (where the sandbox may point) and `governance.py` (what it may do once pointed). `governance.py` is deliberately a thin pass-through to `pipeline.handle_request` rather than a reimplementation — "governance travels with the sandbox" is true precisely because there is nothing new to travel, just the same pipeline imported from a different entry point, so the in-sandbox guarantees are identical by construction. Two sharp edges worth recording: PyYAML parses a workflow's bare `on:` key as the boolean `True`, so the test reads `data.get(True, data.get("on"))` — a gotcha for any YAML-workflow assertion. And the repo's security hook fires on every `.github/workflows/*.yml` write; the fix that satisfies it cleanly is to pass `workflow_dispatch` inputs through a job-level `env:` block and reference `$VARS` in `run:`, never `${{ inputs.* }}` inline — which is the correct injection-safe pattern anyway. Per Convention #6 the customization-guide entry (the allow-list/target surface) landed in this sub-step, not 12.5, because that is when the guards that read the surface shipped; `docs/user-guide/sandbox.md` was created here too so the guide's link resolves (12.5 expands it). The wildcard allow-list matcher needed care — `endswith("." + base)` (not a bare `endswith`) so `notexample.com` does not match `example.com`.
- **Step 12.3 (the six commands).** Factoring `sandbox_support.py` (workspace/config/state I/O + `resolve_provider`) out of the six helpers is the right DRY move — the helpers stay thin and the shared module ships inside the plugin (D18, same `scripts/` dir, importable by the co-located helpers). The testability key is dependency injection: every helper's core function takes `provider=None` and only calls `resolve_provider` (which adds repo root to `sys.path`) when no provider is injected — so tests pass a `MockSandboxProvider` and never trigger the repo-root import or a shell-out. Idempotency is state-driven, not provider-driven: launch checks the persisted `state.json` status and returns early if already running, and stop returns early if already stopped — so the helper enforces idempotency even with a provider that would happily re-provision. `config.yaml` is skip-not-overwrite (a user-editable seed) while `state.json` is overwrite (pure generated state) — the same seed-vs-generated split the project uses elsewhere. No new extensible surface yet (the config the commands write is the surface; its worked example lands at 12.5 with the guards that read the allow-list).
- **Step 12.2 (provider abstraction).** The provider lifecycle is the right seam: all four backends (`docker-compose`, two refusing stubs, the mock) implement one `launch → status → sync → teardown` interface, so the commands (12.3) and the workflow (12.4) never branch on backend. Two patterns carried over cleanly from prior phases — the `web_start` real-shell-out-refused-under-pytest guard (here `SandboxLaunchRefusedError` on a non-dry call) and the `MockAgentAdapter` hermetic-double pattern (here `MockSandboxProvider`, which records calls and is idempotent). The `status()` method is the honest MVP limitation: a live backend query is out of scope, so `DockerComposeProvider.status` returns `status="unknown"` and the command layer (12.3) treats the persisted `state.json` as the source of truth — documented in the docstring rather than faked. No new extensible surface yet (the config schema's worked example lands at 12.5 with the guards).
...
- **Step 13.9 (sign-off — project complete).** The cold-user walkthrough drove a *simulated PR* end to end at two modes against a fresh consuming project — mode 0 produced advice only (impacted `{search, sign-in}`, gap `search`, proposal `search`, no PR, no audit) and mode 3 opened the labeled PR `[test-commander/auto] Add tests for search` with one audit entry — the autonomy ceiling visible in one run. `tests/test_phase_13_signoff.py` (18 tests) landed RED on the five closing markers and GREEN after; its status-line assertion was written robust from the start (`max(Phase N complete) >= 13`), applying the Phase-12 lesson so the project's final status flip needed no prior-phase test fix. Project retrospective: the through-line across all fourteen phases is that Phase 10.5 built the controlled-execution pipeline as the *single* execution path, and every later capability — the web console (10), the Runtime API + MCP server (11), the sandbox (12), and continuous quality (13) — is a thin front-end that inherits the same policy/approval/audit guarantees for the cost of one small wiring module. "Autonomy is a ceiling" cost exactly one `auto_approves`/`can_open_pr` gate plus a `handle_request` call, because there was nothing to bypass. With 13.9 the full roadmap (Phases 0–13) is shipped. No new extensible surface.
- **Step 13.8 (testing finalization).** The cap bump to 13 makes `UNEXPECTED=0` for the *whole* twenty-skill catalog — the integration test pins that invariant explicitly (`max(CATALOG.values()) <= DEFAULT_PHASE_CAP`), which is the right end-of-project assertion: there is nothing ahead of schedule because nothing is left. The PR-simulation test consolidated the loop at all five modes in one parametrized assertion (0–2 → no PR/no audit, 3–4 → labeled PR/one audit), and like Phase 12's it found nothing new — every layer was adversarially tested per sub-step, so the integration pass is confirmation. PyYAML reads JSON, so the `pr.json` bundle could be read with `yaml.safe_load`, but the assertions key on the dict the helper returns rather than re-reading the file, keeping the test about behavior not serialization. No new extensible surface.
- **Step 13.7 (documentation pass).** The `governed-self-improvement.md` "what it does NOT do" section is the load-bearing honesty: an autonomous test-writing agent invites the fear of runaway changes, so stating plainly that it never auto-merges, never bypasses approvals, and never auto-touches secrets/destructive/admin is what makes the capability trustworthy. Per Convention #6 the autonomy-mode config (`continuous/config.yaml`) is the genuinely-extensible surface, documented with a worked example here. The plugin README's "What arrives later" table had quietly gone stale (it still listed Phase-7 skills as future) — the project-complete status surfaced it, so I collapsed it to "all phases shipped"; a reminder that running status tables accrete drift and a final phase should sweep them. Four new docs took the link checker 333 → 337. No code.
- **Step 13.6 (CI workflow).** The honest-MVP guard is what makes this workflow shippable on the plugin repo: it triggers on `push`/`pull_request`, but the plugin repo has no consuming `.test-commander/` workspace, so the check step `if [ ! -d .test-commander ]; then ... exit 0; fi` skips cleanly and CI stays green — the workflow is the *template* a consuming project runs against its own workspace. `permissions: contents: read` is the structural enforcement of "generated changes arrive as gated PRs, never a direct push" — the workflow's token literally cannot push. The diff is computed from trusted git refs (`HEAD~1 HEAD`), and the only input (`autonomy_mode`) flows through `env:`, so there is no injection surface even though this is the first push/PR-triggered workflow in the project. Tested the workflow's shape (triggers, read-only token, runs-the-check, workspace-guard) plus the hermetic "simulated PR runs the check at mode 0" — the Python equivalent of a CI run, asserting advice-only behavior. No new extensible surface yet (the autonomy-mode config's worked example lands at 13.7).
- **Step 13.5 (orchestrator + gates).** The orchestrator is a thin composition of the five already-tested helpers, which is the point of "reuse, don't rebuild" — `check` calls watch/impact/coverage-gap/propose (all read-only) then loops `create_test_pr` over the gaps, gated by `can_open_pr(mode)`. The clean separation is: the *analysis* always runs (advice), only the *PR opening* is mode-gated, so mode 0 is a genuine advisor (full analysis, zero execution, zero audit) and the autonomy ceiling only ever restricts the execute step. The five-mode matrix test (each mode auto-approves exactly its cumulative set, nothing more) is the headline "autonomy is a ceiling" assertion — a single parametrized test over all 5 modes × 6 gated levels is worth more than five separate ones because it proves *exactly* the boundary, not just a sample. With this step the six commands are all shipped, so the SKILL/methodology deferral wording is fully cleared (only the CI-workflow wiring, a non-command, remains for 13.6). No new extensible surface yet (the autonomy-mode config's worked example lands at 13.7).
- **Step 13.4 (propose + gated PR).** The autonomy modes are *cumulative sets*, not a ceiling on the policy level order — the natural progression (advisor → safe-write → execute-tests → code-write → external-network) is not the policy `LEVELS` order (`code-write` < `execute-tests`), so a "ceiling on the linear order" model would grant mode 2 `code-write` (via `execute-tests`) but deny it to mode 3, which is backwards. Explicit per-mode frozensets avoid that trap, and `destructive`/`admin` are absent from every set (a named test). `can_open_pr(mode)` is a *hard* boundary independent of approval — mode 0–2 cannot open a PR even with a human approval flag — because "mode 0 cannot open PRs" is a mode boundary, not an approval question. The "through the pipeline" requirement was satisfied by routing the PR's underlying work (`generate playwright tests for <feature>`) through `handle_request` so it inherits policy + approval + audit, rather than inventing a new PR-creation command in the planner (reuse, don't rebuild). `resolve_repo_root` had to add `runtime/` (not just the repo root) so a standalone helper can import `governance`/`agent_adapters`, which live under `runtime/`. No new extensible surface yet (the autonomy-mode config's worked example lands at 13.7).
- **Step 13.3 (coverage gap).** The "never invents coverage" rule needed an explicit third state: a feature is covered, a gap-because-not-automated, or a gap-because-no-record — the last is the one a naive implementation would silently treat as covered (a `dict.get` miss). Making "unknown feature → gap with reason `no coverage record`" a named test keeps the analysis conservative: it never assumes coverage it cannot cite. Mirrored 13.2's structure (load workspace artifact → deterministic pure function → JSON + Markdown report). The forward-link gotcha recurred (`coverage-gap-analysis.md` → `propose-tests.md`, a 13.4 page) — dropped it, and restored the now-valid `impact-analysis.md` → `coverage-gap-analysis.md` link in the same step. No new extensible surface (read-only analysis).
- **Step 13.2 (watch + impact).** The "never invents impact" property is the load-bearing test: a deterministic prefix-match that accumulates only what the impact map records, so a changed file with no matching pattern (`README.md`) and a mapped-but-unchanged feature (`checkout`) both stay out of the impacted set — provenance makes that auditable. Modeling the impact map as a real `product-knowledge/impact-map.yaml` workspace artifact (not a fixture-only construct) keeps the analysis honest: it reads project knowledge a Phase-3 ingestion would produce. The forward See-also link from `impact-analysis.md` to `coverage-gap-analysis.md` (which lands in 13.3) broke the link checker — a recurring per-command-page gotcha: a command page may only link to sibling pages that already exist, so a cross-link to a later sub-step's page must wait for that sub-step. Sorted the impacted lists for deterministic equality in tests. No new extensible surface (read-only analysis).
...
### Phase 10.5
Phase 10.5 complete (2026-06-01) — see Completed.

### Phase 11

Phase 11 complete (2026-06-02) — see Completed.

### Phase 12

Phase 12 complete (2026-06-02) — see Completed.

### Phase 13

Phase 13 complete (2026-06-02) — see Completed. **Project complete: Phases 0–13 are shipped.**
...
### Phase 13 — Continuous Quality Agent Mode (2026-06-02)

One skill shipped (`tc-continuous-quality`, six `/tc:*` commands) — the final capability. Continuous mode watches application changes, maps them to impacted features and requirements, finds coverage gaps, proposes tests, and opens clearly-labeled pull requests, running the **watch → analyze → propose → PR** loop through the same Phase-10.5 pipeline. `/tc:watch-changes` parses a PR/push diff; `/tc:impact-analysis` maps changed files to impacted features via `product-knowledge/impact-map.yaml` (deterministic, never invents impact); `/tc:coverage-gap-analysis` flags impacted-but-uncovered features against `traceability/coverage.yaml` (never invents coverage); `/tc:propose-tests` proposes BDD/automation for the gaps; `/tc:create-test-pr` opens a labeled PR through the pipeline; `/tc:continuous-quality-check` orchestrates the loop under the configured autonomy mode. **Autonomy is a ceiling:** the five modes (0 read-only-advisor → 4 governed-autonomy) map to which permission levels auto-approve (`continuous/autonomy.py`: `auto_approves`/`can_open_pr`); `destructive`/`admin` never auto-approve, only modes 3–4 open PRs, and nothing above the mode executes without explicit human approval (asserted by the five-mode matrix test). The CI workflow (`.github/workflows/test-commander-continuous-quality.yml`) triggers on `pull_request`/`push`/`schedule`/`workflow_dispatch` with a `contents: read` token — the analysis runs automatically; generated changes arrive only as gated, labeled PRs. Four docs ship (`continuous-quality-agent.md`, `autonomy-levels.md`, `governed-self-improvement.md`, `user-guide/continuous-quality.md`) including the honest "what it does not do" (never auto-merges, never bypasses approvals, never auto-touches secrets/destructive/admin). 1200-test suite green; lint clean; 337-file Markdown link check clean. `verify_skills.py` reports **all twenty skills PRESENT** (`tc-continuous-quality` at phase 13) with `UNEXPECTED=0` — the full catalog. The genuinely-extensible surface (the autonomy mode + PR label) is documented in the customization guide. Tagged `phase-13` on origin. **With this phase the full roadmap (Phases 0–13) is shipped.**

### Phase 12 — Sandboxed Testing Environment (2026-06-02)

One skill shipped (`tc-sandbox`, six `/tc:sandbox-*` commands) — on-demand, team-accessible Test Commander environments launched from GitHub Actions, **governed by the same Phase-10.5 pipeline that runs locally** (sandboxing never relaxes governance). The provider abstraction (`sandbox/providers/`) puts every backend behind one `SandboxProvider` interface (`launch → status → sync → teardown`): the MVP default is docker-compose-local (lifecycle calls plan deterministically in dry-run; a real launch shells out and is refused under pytest), with a generic-container-host and a Sprites.dev placeholder as refusing stubs (Q8 default) and a `MockSandboxProvider` for hermetic testing. The six commands are self-contained plugin helpers (D18) that manage `<workspace>/.test-commander/sandbox/{config.yaml,state.json}` and drive the provider; launch and stop are state-driven idempotent. Safe-by-default targeting (`sandbox/safety.py`): allow-listed hosts only, private/loopback/link-local ranges blocked. Governance travels via `sandbox/governance.py` (`run_in_sandbox` → `pipeline.handle_request`) — a sandbox cannot execute above its approved level, asserted by tests. The GitHub Actions workflow (`.github/workflows/test-commander-sandbox.yml`) is manual-dispatch only, dry-run by default, sequencing safety → build → publish → teardown with inputs passed through `env:` (injection-safe). Four docs ship with an **honest MVP-limitations** section (docker-compose-local only, persisted-state status, demonstrative CI steps, no real spend in tests). 1110-test suite green; lint clean; 323-file Markdown link check clean. `verify_skills.py` reports all nineteen skills `PRESENT` (`tc-sandbox` at phase 12) with `UNEXPECTED=0`. The genuinely-extensible surface (the sandbox config's allow-list, target, and approvals) is documented in the customization guide. Tagged `phase-12` on origin.

### Phase 11 — Runtime API and MCP Server (2026-06-02)

One skill shipped (`tc-mcp`, no `/tc:*` commands) — two alternative front-ends to the Phase-10.5 governance pipeline. The **Runtime API** (`apps/api`) expands the Phase-10 read-only console API with the `/api/runtime/` namespace: `GET /info` (the seven permission levels), `POST /plan` (a read-only dry run via the new `pipeline.preview` — routes, plans, and classifies a request and reports the level + role allowance, with no execution and no audit entry), and `POST /execute` (governed execution through `pipeline.handle_request`). The **MCP server** (`apps/mcp`) is a lightweight, schema-first registry — no heavy SDK — of three tools (`tc_status`, `tc_plan`, `tc_run_command`); its `dispatch()` handles the `initialize`/`tools/list`/`tools/call` protocol and is wrapped by a thin stdio entry point. Both front-ends enter the same pipeline server-side, so the seven permission levels are enforced identically and there is no direct-execution backdoor: a denied request is blocked before the agent, a destructive request is refused for an unauthorized role, a client cannot escalate by supplying a `level`, and an approval with no `approver` is held. The phase hardened the shared pipeline twice via adversarial tests — an unattributed approval no longer executes (11.4), and a privileged-classified-but-unrouted request (`delete all evidence`, which maps to no `/tc:*` command) no longer "executes" a no-op (11.6); both were one-line fixes both front-ends inherited. 1050-test suite green; lint clean; 309-file Markdown link check clean. `verify_skills.py` reports all eighteen skills `PRESENT` (`tc-mcp` at phase 11) with `UNEXPECTED=0`. Phase 11 adds no new extensible surface (the front-ends inherit the existing `policy/permissions.yaml` and `approvals.yaml`); the only configuration is deployment-level (`TC_WORKSPACE` and the adapter choice). Tagged `phase-11` on origin.

### Phase 10.5 — Controlled agent execution and policy-governed chat (2026-06-01)

One skill shipped (`tc-governance`, no `/tc:*` commands) — the controlled-execution pipeline that makes the web console safe to expose. Every request flows through `governance.pipeline.handle_request`: **intent router** (maps to a known `/tc:*` or read-only; cannot synthesize a command) → **command planner** (a deterministic, displayable plan) → **permission policy** (seven levels × five roles, default deny) → **approval gate** (privileged levels require approval; the card + an `audit/approvals/` record) → **bounded executor** (a structured instruction built only from the plan — never raw user text — run through an `AgentAdapter`) → **output validation** (the diff must match the planned scope; no secret file; expected outputs produced) → **audit journal** (one append-only `audit/actions.jsonl` entry). The `AgentAdapter` abstraction (`MockAgentAdapter`, the real gated `ClaudeCodeCliAdapter`, a stubbed `AnthropicApiAdapter`) keeps governance backend-agnostic; the only path to execution is the pipeline, and the Phase-10 console's single `/api/execute` route runs through it (chat and proposals stay read-only). The phase was driven by **four canonical security integration tests written RED first** (block-before-agent, deny-no-change, approve-diff-matches, no-plan-bypass), each flipped GREEN at its enabling sub-step via `xfail(strict=True)`. 989-test suite green; lint clean; 303-file Markdown link check clean. `verify_skills.py` reports all seventeen skills `PRESENT` (`tc-governance` at phase 10.5) with `UNEXPECTED=0`. The genuinely-extensible surface (roles + approval policy) is documented in the customization guide. Tagged `phase-10.5` on origin.

- [x] Steps 10.5.1-10.5.13: scaffold + the four RED security tests; `AgentAdapter` + mock + stubs; permission policy engine; intent router; command planner; approval gate; bounded executor; output validation + secret safety; audit journal; `ClaudeCodeCliAdapter` + console `/api/execute` wiring; six governance docs; cap bump 10 → 10.5 + the consolidated integration smoke; sign-off + `phase-10.5` tag. Each sub-step landed as one commit with the security spine flipping green at 10.5.3 / 10.5.6 / 10.5.8 / 10.5.9.
