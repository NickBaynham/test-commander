...
- Phase 12 ships sandboxes (`tc-sandbox`): `/tc:sandbox-init`, `/tc:sandbox-launch`, `/tc:sandbox-status`, `/tc:sandbox-sync`, `/tc:sandbox-stop`, `/tc:sandbox-export` — on-demand, team-accessible Test Commander environments launched from GitHub Actions, governed by the same Phase-10.5 pipeline and safe-by-default (allow-listed hosts, blocked private ranges). See [sandbox.md](sandbox.md).
- Phase 13 ships continuous quality (`tc-continuous-quality`): `/tc:watch-changes`, `/tc:impact-analysis`, `/tc:coverage-gap-analysis`, `/tc:propose-tests`, `/tc:create-test-pr`, `/tc:continuous-quality-check` — watches changes, maps impact, finds coverage gaps, and opens clearly-labeled test PRs when the configured autonomy mode (0–4) allows, all through the same Phase-10.5 pipeline. See [continuous-quality.md](continuous-quality.md).
