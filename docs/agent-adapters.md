...
The base interface lives at `runtime/agent_adapters/base.py`. Concrete adapters live alongside. As shipped in Phase 10.5:
...
- `BoundedInstruction` (frozen) — the structured input the runtime built: `command`, `scope`, `project_root`, `allowed_paths` / `disallowed_paths`, `allowed_actions` / `disallowed_actions`, `expected_outputs`, `safety_rules`, `journal_required`, `approved`.
- `ExecutionResult` — `status`, `files_changed`, `artifacts_created`, `usage`, `summary`, `events`.
- `UnplannedExecutionError` — raised by the shared `require_planned()` guard when an adapter is called without an **approved** `BoundedInstruction`. Every adapter routes `execute_command`/`stream_events` through it, so there is **no direct-execution backdoor**.
