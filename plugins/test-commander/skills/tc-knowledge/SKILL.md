...
Phase 3 is in progress. The skill scaffold and seeded sample-project fixture (Step 3.1) have shipped. Steps 3.2-3.4 have shipped `/tc:learn-from-docs` (with the shared `synthesize_system_model.py` helper), `/tc:learn-from-specs`, and `/tc:learn-from-code`. Command behavior arrives in subsequent sub-steps:
