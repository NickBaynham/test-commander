...
- **Governed-execution routes** run an approved request through the pipeline
  (`POST /api/execute`). This is the only path that can change the workspace or
  run a command, and it enters `governance.pipeline.handle_request` —
...
(Behavior shipped in Step 11.2; this methodology page is the scaffold spec.)
