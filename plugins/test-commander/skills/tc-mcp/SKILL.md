...
Phase 11 (Step 11.4). The Runtime API, the MCP server, and the server-side permission gates are shipped:
...
- Server-side enforcement of the seven permission levels across API + MCP — **shipped (Step 11.4).** Both front-ends enter the same pipeline, so both enforce identically: a per-level matrix proves each level permits its role and denies a role that lacks it (default deny); a destructive action is refused for an unauthorized role on both the Runtime API and the MCP tool; a client cannot escalate by supplying a `level` (the server classifies from the request). The approval gate was hardened — **an approval with no approver is not an approval** (a privileged action with `approve=true` but no `approver` is held, not executed), closing an accountability bypass. See [methodology/mcp-tools.md](methodology/mcp-tools.md).
