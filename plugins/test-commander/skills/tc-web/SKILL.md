...
Phase 10 (Step 10.6). All five commands are end-to-end runnable:
...
- `/tc:web-export` — **shipped (Step 10.6).** Exports the console view (quality facts, requirements, runs, evidence, traceability) as a deterministic static bundle (`data.json` + `index.html`) under `.test-commander/.web/export/`. Read-only; refuses an uninitialized workspace (exit 2). Full spec: [commands/web-export.md](commands/web-export.md).
