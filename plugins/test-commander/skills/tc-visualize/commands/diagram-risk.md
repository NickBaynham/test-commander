...
Render the risk register grouped by severity.
...
- `<workspace>/visuals/mermaid/risk.md` — a Mermaid `flowchart` with a severity subgraph per level, plus a `> Sources:` footer.

## Preconditions

- The workspace exists (`/tc:init` has run). Otherwise exit 2.
- The source is populated. A missing source is refused (exit 2) pointing at `/tc:review-requirements`.
...
3. Render via the shared engine and write `visuals/mermaid/risk.md`.
...
- Writes only `visuals/mermaid/risk.md`. Reads the source; never modifies it.
- No network, no browser.

## Implementation

- Helper: `plugins/test-commander/scripts/visualize.py` — `diagram-risk` (underscored).
