...
Phase 13 (Step 13.6). All six commands and the CI workflow are shipped:
...
- The continuous-quality CI workflow — **shipped (Step 13.6).** `.github/workflows/test-commander-continuous-quality.yml` triggers on `pull_request` / `push` / `schedule` / `workflow_dispatch` and runs the check at the configured autonomy mode. The token is `contents: read` — the read-only analysis runs automatically, and any generated change arrives only as a gated, labeled PR, never a direct push. The check step guards on a `.test-commander/` workspace and skips cleanly where there is nothing to analyze.
