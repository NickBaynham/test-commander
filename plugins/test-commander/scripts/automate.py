...
// Test data reaches the spec only through this fixture (D6); the page object owns
// locators. Specs pair a page object with this fixture - the framework's best
// practice (methodology/playwright-standards.md). For database-layer assertions,
...
export const test = base.extend<{{ data: {name}Data; resetState: void }}>({{
  // Auto: reset the app to a known state before every test when a reset endpoint
  // is configured (PLAYWRIGHT_RESET_PATH). No-op otherwise. Keeps tests isolated on
  // a shared backend whose database persists between tests.
  resetState: [
    async ({{ request }}, use) => {{
